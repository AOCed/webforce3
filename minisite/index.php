...
<html lang="fr">
	...
	<body>
		...
		<table>
			...
			<?php  
				$sql = "SELECT * FROM products order by pr_price ASC";
				...
				while ($datas = mysqli_fetch_assoc($req)) {
					...
					if(isset($_SESSION['user']['type']) && ($_SESSION['user']['type'] == "admin")) {
						echo "<td><a href='add_modify_products.php?id=".$datas['pr_id']."' id='ajoutProduit'>modifier</a></td>";
					}
					if(isset($_SESSION['user']['type']) && ($_SESSION['user']['type'] == "admin")) {
						echo "<td><a href='libs/services_products.php?action=delete&id=".$datas['pr_id']."' id='ajoutProduit'>Supprimer</a></td>";
					}
				echo "</tr>";
				}
			?>
